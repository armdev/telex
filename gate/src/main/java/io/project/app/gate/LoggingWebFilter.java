...
public class LoggingWebFilter implements WebFilter {
    ...
    public Mono<Void> filter(ServerWebExchange exchange, WebFilterChain chain) {
        log.info("XXXXXXXXXXXXXXs347823489349");
    ...
    private ServerHttpRequest getDecoratedRequest(ServerHttpRequest request) {
        ...
        return new ServerHttpRequestDecorator(request) {
            ...
            public Flux<DataBuffer> getBody() {
                log.info("Request: requestId: {}, method: {} , url: {}", request.getId(), request.getMethod(), request.getURI());
                return super.getBody().publishOn(Schedulers.boundedElastic()).doOnNext(dataBuffer -> {
                    try (ByteArrayOutputStream baos = new ByteArrayOutputStream()) {
                        ...
                        log.info("Request body: for requestId: {}, request body :{}", request.getId(), body);
                    } catch (Exception e) {
